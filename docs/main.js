...
html += '<label>Tipo de neumático:<input type="text" id="tipoNeumatico"/></label>';
html += '<label>Km que xa tiñan:<input type="number" id="kmPrevios"/></label>';
...
function guardarRegistro() {
  ...
  const r = {
    tipoNeumatico: $("tipoNeumatico")?.value || "",
    kmPrevios: $("kmPrevios")?.value || "",
    tipo,
    fecha,
    km: $("km")?.value || "",
    litros: $("litros")?.value || "",
...
function renderResumen() {
  ...
  datos.registros.slice().reverse().forEach(r => {
    lista += `<p>📅 ${r.fecha} - ${r.tipo} - ${r.km || "—"} km - ${r.litros || "—"} L - ${r.coste || "—"} € - ${r.detalle || ""}`;
    if (r.tipo === "neumatico") {
      lista += ` - Tipo: ${r.tipoNeumatico || "—"} - Km previos: ${r.kmPrevios || "0"}`;
    }
    lista += `</p>`;
  });
